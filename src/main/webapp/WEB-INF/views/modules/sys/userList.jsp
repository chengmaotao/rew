...
<body>
	...
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/list" method="post" class="breadcrumb form-search ">
		...
		<ul class="ul-form">
			...
			<li><label>参与初审：</label>
				<form:select path="iscs" cssClass="input-medium">
			...
			<li><label>参与复审：</label>
				<form:select path="isfs" cssClass="input-medium">
					<form:option value="">请选择</form:option>
					<form:options items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="user">
			<tr>
				...
				<td>${fns:getGroupById(user.groupId).groupname}</td>
				<td>${fns:getRolesByUserId(user.id)}</td> 
				<td>${user.iscs == '1' ? '参与' : '不参与' }</td>
				<td>${user.isfs == '1' ? '参与' : '不参与' }</td>
				<td>${user.remarks }</td>
				<td>
