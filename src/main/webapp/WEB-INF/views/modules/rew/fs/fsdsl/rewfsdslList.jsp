...
<c:if test="${isfs }">
    <c:if test="${isShow }">
    	<form:form id="searchForm" modelAttribute="rewfsdyl" action="${ctx}/fs/fsdyl/initrewfsdyl/list3" method="post" class="breadcrumb form-search">
    		<ul class="ul-form">
	    		<li><label>项目名称：</label> 
	...
	<c:if test="${!isShow }"><shiro:hasPermission name="fsdslpszj"><label style="text-align: right; display: block; margin-right: 20px"> <b>投票总数：${allNum } /已投票数：${fn:length(fsdyltpNums)}</b></label></shiro:hasPermission></c:if>
	<input type="hidden" value="${num }" id="synumId"> <%--还可以提交的数目 --%>
	<form action="${ctx}/fs/fsdyl/initrewfsdyl/saveAllData3" method="post" id="myFormId" name="myFormName">
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed">
			<thead>
				<tr>
				    ...
				    <c:choose>
				    	<c:when test="${isShow }">
				    		<shiro:hasPermission name="fsdslgly"><th>评审专家</th></shiro:hasPermission>
				    		<shiro:hasPermission name="fsdslgly"><th>同意授二等奖</th></shiro:hasPermission>
				    	</c:when>
				    	<c:otherwise>
				    		<shiro:hasPermission name="fsdslpszj"><th>同意授二等奖</th></shiro:hasPermission>
				    	</c:otherwise>
				    </c:choose>
			...
			<tbody>
				<c:forEach items="${rewfsdslList}" var="rewfsdyl" varStatus="s">
					<tr>
						...
						<c:choose>
				    		<c:when test="${isShow }">
				    			<shiro:hasPermission name="fsdslgly"><td>${fns:getUserById(rewfsdyl.createBy).name}</td></shiro:hasPermission>
				    			<shiro:hasPermission name="fsdslgly"><td>${rewfsdyl.isAgree =="1" ? "同意" : ""}</td></shiro:hasPermission>
				    		</c:when>
				    		<c:otherwise>
					    		<shiro:hasPermission name="fsdslpszj">
									<td>
										<c:choose>
		...
		<c:if test="${!isShow and fn:length(rewfsdslList) > 0}">
			<shiro:hasPermission name="fsdslpszj">
