...
<c:if test="${isfs }">
    <c:if test="${isShow }">
    	<form:form id="searchForm" modelAttribute="rewfsdyl" action="${ctx}/fs/fsdyl/initrewfsdyl/list2" method="post" class="breadcrumb form-search">
    		<ul class="ul-form">
	    		<li><label>项目名称：</label> 
	...
	<c:if test="${!isShow }"><shiro:hasPermission name="fsdelpszj"><label style="text-align: right; display: block; margin-right: 20px"> <b>投票总数：${allNum } /已投票数：${fn:length(fsdyltpNums)}</b></label></shiro:hasPermission></c:if>
	<input type="hidden" value="${num }" id="synumId"> <%--还可以提交的数目 --%>
	<form action="${ctx}/fs/fsdyl/initrewfsdyl/saveAllData2" method="post" id="myFormId" name="myFormName">
				...
				<tr>
				    ...
				    <c:choose>
				    	<c:when test="${isShow }">
				    		...
				    		<shiro:hasPermission name="fsdelgly"><th>同意授一等奖</th></shiro:hasPermission>
				    	</c:when>
				    	<c:otherwise>
				    		<shiro:hasPermission name="fsdelpszj"><th>同意授一等奖</th></shiro:hasPermission>
				    	</c:otherwise>
				    </c:choose>
				</tr>
			</thead>
		...
		<c:if test="${!isShow and fn:length(rewdsdelList) > 0}">
			<shiro:hasPermission name="fsdelpszj">
