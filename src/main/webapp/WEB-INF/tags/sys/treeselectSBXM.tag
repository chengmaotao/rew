		...
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}", "选择${title}", 300, 420, {
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				if (v=="ok"){
					...
					function checkProjectId(uid){
					  var oldProjectId = $("#oldProjectId").val();
				      $.ajax({  
				           type: "post",
				           url: "${ctx}/zszj/rewidea/checkProjectId", 
				           data:  {"oldProjectId":oldProjectId,"projectid":uid},
				           success: function(msg){
						       if(msg == "true"){
						           // 项目编号
								   $("#tempProjectId").val($("#${id}Id").val());
						           findProjectById($("#${id}Id").val());
						       }else{
						           $("#${id}Id").val("");
								   $("#${id}Name").val("");
						       	   alert("该项目评价意见已经存在，如想再次评价，请在列表页面点击修改。");
						       }
				           } ,
					...
					checkProjectId($("#${id}Id").val());
					
					
					
					function findProjectById(uid){
				      $.ajax({  
				           type: "post",
				           url: "${ctx}/zszj/rewidea/projectMess", 
				           data:  {"projectId":uid},
				           success: function(msg){
				              // 专业组
				              $("#tempGroupId").val(msg.group);
				              
				              // 申报单位
		                      $("#tempCompanyId").val(msg.company);
				           } ,
				           error: function (XMLHttpRequest, textStatus, errorThrown) { 
				                       alert(errorThrown); 
				           } 
